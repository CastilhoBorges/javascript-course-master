...
const checkDogs = function (dogsJulia, dogsKate) {
  const juliaCopyData = dogsJulia.slice();
  juliaCopyData.splice(0, 1);
  juliaCopyData.splice(-2);

  const dogs = [...juliaCopyData, ...dogsKate];
...
const movUSD = mov.map((mov) => mov / realToDolar);
console.log(mov);
console.log(movUSD);
...
const calcAverageHumanAge = function (arr) {
  const ageDogForAgeHuman = arr
    .map((age) => (age <= 2 ? age * 2 : age * 4 + 16))
    .filter((age) => age >= 18);

  const reduceAges = ageDogForAgeHuman.reduce((acc, age) => (acc + age), 0);

  return reduceAges / ageDogForAgeHuman.length;
};
...
const dolarToReal = 5.5;
const total = movements
  .filter((mov) => mov > 0)
  .map((mov) => mov * dolarToReal)
  .reduce((acc, mov) => acc + mov, 0);
console.log(total);
*/
...
const mov = movements.find(mov => mov < 0);
...
const davis = accounts.find((acc) => acc.owner === "Jessica Davis");
console.log(davis);
